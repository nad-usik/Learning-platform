...
def teacher_dashboard(request):
    if request.user.is_authenticated:
        ...
        # lessons = LessonSlot.objects.filter(teacher_id=teacher.id)
        ...
        if not lessons:
            ...
            is_available = True
        return render(request, 'teacher_dashboard.html', {'profile': profile, 'lessons': lessons, 'is_available': is_available})
    else:
...
def teacher_calendar(request):
    ...
    # context = {'hours': hours, 'call': 'calendar', 'profile': profile}
